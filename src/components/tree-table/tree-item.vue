	...
	<div :class="tdClass">
		...
		<table>
			<tr>
				<td :colspan="colSpan">
					<table>
						<tr class="leve" :class="levelClass" @click="rowClick">
							<td class="td1">
								<div class="td-title" @dblclick="handlerExpand(model)">
									<span v-if="model.children.length > 0" class="tree-close"
										:class="{'tree-open':model.isExpand}" @click="handlerExpand(model)"></span>
									<a class="ellipsis">
							...
							<td class="td4">
								<span :title="model.revisor" class="ellipsis">{{model.revisor}}</span>
							</td>
							<td class="td5">{{model.pulishTime}}</td>
							<td class="td6">
...
<script>
	export default {
		...
		created() {
			// console.log(this.model, 9876554)
		},
