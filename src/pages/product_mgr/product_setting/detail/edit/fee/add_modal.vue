  ...
  <div>
    ...
    >
      <div class="modal-inner">
        <el-form
          v-if="submitData.productTermParas"
          label-position="left"
          :inline="true"
          label-width="150px"
        >
          <el-form-item label="费用项名称" required>
            <el-select placeholder="请选择" v-model.trim="submitData.termName" @change="feeChange">
              <el-option
          ...
          <el-form-item label="费用项ID" required>
            <el-input
              placeholder
              disabled
              v-model.trim="submitData.productTermParas[0].defaultValue"
            ></el-input>
          </el-form-item>
          ...
          <el-form-item v-show="ifShowEarlyRepayment" label="提前还款减免比例" required>
...
export default {
  ...
  mounted() {
    ...
    getAddStruct().then(res => {
      ...
      this.submitData.productTermParas = JSON.parse(
        JSON.stringify(this.TERM_PARAMS)
      );
    });
    //获取费用项名称列表
    getFeeSelectList().then(res => {
  ...
  watch: {
    ["submitData.termName"](newV) {
      this.feeList.length &&
        this.feeList.forEach(item => {
          if (item.termName === newV) {
            this.submitData.termId = item.termId;
            this.submitData.productTermParas[0].defaultValue =
              item.productTermParas[0].defaultValue;
          }
        });
    }
  },
  ...
  methods: {
    ...
    feeChange(val) {
      if (this.submitData.termName === "管理费") {
        this.submitData.productTermParas[1].defaultValue = "1";
        this.submitData.productTermParas[2].defaultValue = "01";
        this.submitData.productTermParas[3].defaultValue = "01";
        this.submitData.productTermParas[7].defaultValue = "03";
      }
    },
    //提前还款比例改变
    ...
    isEmpty() {
      ...
      if (
        this.submitData.termName == "VIP包" &&
        !this.submitData.EarlyRepayment
      ) {
    ...
    submit() {
      ...
      this.submitData.objectNo = this.objectNo;
      this.submitData.productTermParas.forEach(item => {
        item.objectNo = this.objectNo;
        item.termId = this.submitData.termId;
      });
      if (!this.checkForm()) {
        return;
      }
      ...
      addProductFee(this.submitData).then(res => {
        this.modalClose();
      });
    },

    modalClose() {
      this.submitData.productTermParas = TERM_PARAMS;
