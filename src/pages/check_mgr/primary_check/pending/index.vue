<template>
  <div class="container">
    ...
    <el-table :data="tableData" border class="table" :height='tableHeight' ref="multipleTable" highlight-current-row
      ...
      <el-table-column prop="contractStatusMain" label="合同主状态" min-width="110" align="center"></el-table-column>
      <el-table-column prop="contractStatusMinor" label="合同子状态" min-width="110" align="center"></el-table-column>
    ...
    <!-- 签署意见 -->
    <SigningModel m_title='签署意见' :isShowModal='isShowModal' source='审批初审' :columnInfo='selection'></SigningModel>
    <!-- 提交审核 -->
    <ExamineModel m_title='提交审核' :isShowModal='isShowModal_E' source='审批初审' :serialNoList='serialNoList'
      :columnInfo='selection'></ExamineModel>
    <!-- 查看意见 -->
    <SeeViewModel m_title='查看意见' :isShowModal='isShowModal_S' source='审批初审' :columnInfo='selection'></SeeViewModel>
    <!-- 个人状态 -->
    <PstatusModel m_title='个人状态' :isShowModal='isShowModal_P' source='审批初审' :columnInfo='selection'></PstatusModel>
    <!-- 查看调单记录 -->
    <RecordModel m_title='查看调单记录' :isShowModal='isShowModal_C' source='审批初审' :columnInfo='selection'></RecordModel>
...
<script>
  ...
  import RecordModel from "@/components/public/record_model";
  import SearchForm from "@/components/public/searchForm";


  export default {
    name: 'check_primary_pending',
    components: {
      SigningModel,
      ExamineModel,
      SeeViewModel,
    ...
    methods: {
      ...
      filterHand(resHandlName) {
        ...
        if (resHandlName != 'onSeeStatus' && Object.keys(this.selection).length == 0) {
          this.$message.warning("请选择需要处理的合同")
          return;
        }

        this.serialNoList = [this.selection.serialNo];
